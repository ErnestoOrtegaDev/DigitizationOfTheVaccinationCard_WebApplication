...
export const CartillaTable = ({ records, patientId }) => {
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            {/* Nuevo contenedor para habilitar el scroll horizontal */}
            <div className="overflow-x-auto">
                {/* Se agregó min-w-[800px] para forzar el scroll en pantallas pequeñas */}
                        ...
                        {Object.entries(groupedRecords).map(([vaccineName, doses]) => (
                            doses.map((record, index) => (
                                <tr key={record.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="p-4 text-sm border-r border-slate-200 whitespace-nowrap">{record.scheme_dose.dose_name}</td>
                                    <td className="p-4 text-sm border-r border-slate-200 whitespace-nowrap">{record.scheme_dose.apply_at_months} meses</td>
                                        ...
                                        {record.status === 'applied' ? (
                                            ...
                                            <div className="relative group inline-block">
                                                <button 
                                                    ...
                                                    className="bg-blue-900 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-800 transition-colors disabled:bg-slate-400 disabled:cursor-not-allowed whitespace-nowrap"
                                                    title={getDisabledReason(record, doses) || 'Registrar Dosis'}
                                                >
                                                    Registrar Dosis
                                                </button>
                                                {getDisabledReason(record, doses) && (
                                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-slate-900 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                                                        {getDisabledReason(record, doses)}
                                                    </div>
                                                )}
                                            </div>
